...
Скрипти для відтворення (golden dataset — `tests/golden_dataset.json`).
Перед запуском переконайтесь, що база знань уже завантажена (розділ
«Завантаження документа в базу знань») і `.env` містить робочий
`OPENAI_API_KEY`.

```bash
python -m tests.evaluate_rag           # retrieval: hit@3, recall@3, precision@3
python -m tests.evaluate_generation    # generation: faithfulness, correctness (LLM-as-judge)
```

Результати, інтерпретація метрик і порівняння версій промпту (v1 vs v2) —
в окремому файлі з письмовими відповідями (Частина C і D).
